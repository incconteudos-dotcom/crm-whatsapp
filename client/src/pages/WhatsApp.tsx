...
import {
  ...
  Phone, UserPlus, QrCode, Info, ChevronLeft,
} from "lucide-react";
...
export default function WhatsApp() {
  ...
  // Mobile: true = mostrando lista, false = mostrando chat
  ...
  const deleteChatMutation = trpc.whatsapp.deleteChat.useMutation({
    onSuccess: () => { toast.success("Chat excluído."); setSelectedChat(null); setMobileShowList(true); refetchChats(); },
    onError: (e) => toast.error(e.message),
  });
  ...
  function handleSelectChat(jid: string) {
    setSelectedChat(jid);
    setMobileShowList(false);
    if ((chats?.find(c => c.jid === jid)?.unreadCount ?? 0) > 0) {
      readChatMutation.mutate({ phone: jid });
    }
  }
        ...
        <div className="flex-1 overflow-y-auto">
          {chatsLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full p-8 text-center">
              ...
              >
                <RefreshCw className={cn("w-3.5 h-3.5 mr-2", syncAllChatsMutation.isPending && "animate-spin")} />
                Buscar do WhatsApp
              </Button>
            </div>
          )}
          ...
          {contactsLoading ? (
            ...
            zapiContacts.map((c) => (
              <div key={c.phone} className="flex items-center gap-3 p-3 border-b border-border/50 hover:bg-accent transition-colors">
                ...
                <button
                  onClick={() => { handleSelectChat(c.phone); setActiveTab("chats"); }}
                  className="text-muted-foreground hover:text-green-400 transition-colors p-1"
          ...
          {groupsLoading ? (
            ...
            groups.map((g) => (
              <div key={g.phone} className="flex items-center gap-3 p-3 border-b border-border/50 hover:bg-accent transition-colors">
                ...
                <button
                  onClick={() => { handleSelectChat(g.phone); setActiveTab("chats"); }}
                  className="text-muted-foreground hover:text-green-400 transition-colors p-1"
                >
                  <MessageSquare className="w-3.5 h-3.5" />
                </button>
          ...
          <div className="h-14 border-b border-border flex items-center justify-between px-3 bg-card shrink-0">
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold text-foreground truncate">{selectedChatData?.name ?? selectedChat}</p>
                <p className="text-xs text-muted-foreground truncate hidden sm:block">{selectedChat}</p>
              </div>
            </div>
            ...
            {(messagesLoading || syncChatMessagesMutation.isPending) ? (
              ...
              messages.map((msg) => (
                <div key={msg.id} className={cn("flex group", msg.isFromMe ? "justify-end" : "justify-start")}>
                  <div className={cn(
                    "max-w-[80%] sm:max-w-[70%] rounded-2xl px-3 py-2 relative",
                    msg.isFromMe
  ...
  return (
    <CRMLayout>
      ...
      <Dialog open={analysisOpen} onOpenChange={setAnalysisOpen}>
        <DialogContent className="max-w-2xl bg-card border-border mx-4">
          <DialogHeader>
